...
namespace CulLang
{
	...
	{
	public:
		Token(const Position &posStart, const Position &posEnd, TokenType type_, const std::string &a = "")
		...
		std::array<Position, 2> getPos() const
		{
		...
		static bool checkIfEqual(const Token *v, TokenType t, bool countNull = false);
		static bool checkIfNotEqual(const Token *v, TokenType t, bool countNull = true);
...
namespace CulLang
{
	Ref<Object> Token::getValueConverted() const
	{
		...
		default:
			...
			ErrorHandler::raiseError({FeatureNotImplementedYet, "This type is not implemented yet for The class 'Tokens'"});
			return Object::NONE;
		}
	}
